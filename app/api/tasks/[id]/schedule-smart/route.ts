  ...
  try {
    ...
    if (!task.due_date) {
      return NextResponse.json(
        { error: "Task must have a due date to use Smart Schedule" },
        { status: 400 }
      );
    }
    ...
    // If task has subtasks, schedule each subtask in reverse order (backward from due date)
    if (subtasks.length > 0) {
      ...
      if (task.group_id) {
        ...
        if (groupResult.rows.length > 0) {
          taskGroup = mapRowToGroup(groupResult.rows[0]);
        }
      }
      ...
      return NextResponse.json({
        ...
        message:
          shuffledTasks.length > 0
            ? `Scheduled ${scheduledSubtasks.length} of ${subtasks.length} subtasks with ${shuffledTasks.length} task(s) shuffled.`
    ...
    if (!result.slot) {
      return NextResponse.json(
        {
          error: result.error || "Unable to schedule task before its due date.",
    ...
    return NextResponse.json({
      ...
      message:
        shuffledTasks.length > 0
          ? `Task scheduled with ${shuffledTasks.length} task(s) displaced to make room.`
          : "Task scheduled successfully.",
    });
  } catch (error) {
    console.error("Error scheduling task with Smart Schedule:", error);
    return NextResponse.json(
      {
