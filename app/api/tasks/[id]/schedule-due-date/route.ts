...
// POST /api/tasks/[id]/schedule-due-date - Schedule a task to its due date (searching backwards)
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (subtasks.length > 0) {
      ...
      // For subtasks, we'll use the parent's due date as the deadline
      // Schedule each subtask backwards from the parent's due date
      const parentDueDate = task.due_date ? new Date(task.due_date) : null;
      if (!parentDueDate) {
        return NextResponse.json(
          { error: "Parent task must have a due date to schedule subtasks" },
          { status: 400 }
        );
      }

      // Schedule each subtask sequentially, working backwards from due date
      // Start from the due date and work backwards
      for (let i = subtasks.length - 1; i >= 0; i--) {
        ...
        // Use unified scheduler with "due-date" mode
      ...
      // If the parent's deadline was already past, let it follow the last subtask end
      // so the parent doesn't reappear as an "overdue deadline".
      const updatedAt = new Date().toISOString();
      const lastSubtaskEnd = latestScheduledEnd(scheduledSubtasks);
      const alignedParentDueDate = lastSubtaskEnd
      ...
      await db.execute(
        ...
        [alignedParentDueDate, updatedAt, id, session.user.id]
      );
      ...
      return NextResponse.json(
        {
          error:
            result.error ||
            "Unable to schedule task. No available time slot found before due date.",
    ...
    console.error("Error scheduling task to due date:", error);
