...
// POST /api/tasks/[id]/schedule-next-week - Schedule a task next week onwards
  ...
  try {
    ...
    if (subtasks.length > 0) {
      ...
      return NextResponse.json({
        ...
        feedback,
        message: `Scheduled ${scheduledSubtasks.length} of ${subtasks.length} subtasks for next week`,
      });
    }
    ...
    if (!result.slot) {
      return NextResponse.json(
        {
          error: result.error || "Unable to schedule task for next week. Please try another mode.",
    ...
    return NextResponse.json({
      task: updatedTask,
      message: "Task scheduled for next week",
      feedback: result.feedback,
    });
  } catch (error) {
    console.error("Error scheduling task for next week:", error);
