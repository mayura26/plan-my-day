...
// POST /api/tasks/[id]/schedule-next-month - Schedule a task next month onwards
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (subtasks.length > 0) {
      ...
      const updatedAt = new Date().toISOString();
      const parentDueDate = lastScheduledEnd
        ? alignDueDateOnReschedule(task.due_date, lastScheduledEnd)
        : (task.due_date ?? null);
      await db.execute(
      ...
      return NextResponse.json({
        ...
        feedback,
        message: `Scheduled ${scheduledSubtasks.length} of ${subtasks.length} subtasks for next month`,
      });
    }
    ...
    // Use unified scheduler with "next-month" mode
    ...
    if (!result.slot) {
      return NextResponse.json(
        {
          error: result.error || "Unable to schedule task for next month. Please try another mode.",
    ...
    return NextResponse.json({
      task: updatedTask,
      message: "Task scheduled for next month",
      feedback: result.feedback,
    });
  } catch (error) {
    console.error("Error scheduling task for next month:", error);
