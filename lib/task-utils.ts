import type { CalendarEvent, Task, TaskStatus, TaskType } from "./types";
...
export const STATUS_COLORS = {
  pending:
    "text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 border-gray-200 dark:border-gray-700",
  in_progress:
    "text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800",
  completed:
...
export function taskToCalendarEvent(task: Task): CalendarEvent {
  ...
  const end = task.scheduled_end
    ...
    : new Date(start.getTime() + (task.duration || 60) * 60000);
  ...
  return {
    ...
    color: task.group_id
      ? undefined
      : getTaskPriorityColor(task.priority).split(" ")[0].replace("text-", ""),
...
export function validateTaskData(data: Partial<Task>): string[] {
  ...
  if (
    data.energy_level_required &&
    (data.energy_level_required < 1 || data.energy_level_required > 5)
  ) {
