...
export function createDateInTimezone(
  ...
  for (let offsetHours = -14; offsetHours <= 14; offsetHours++) {
    ...
    if (tzYear === year && tzMonth === month && tzDate === date && tzHour === 0 && tzMinute === 0) {
      ...
      if (vTzYear === year && vTzMonth === month && vTzDate === date && vTzHour === hours && vTzMinute === minutes) {
        return result
      }
    }
  }
  ...
  console.warn(`Could not find exact timezone match for ${year}-${month + 1}-${date} ${hours}:${minutes} in ${timezone}, using fallback`)
